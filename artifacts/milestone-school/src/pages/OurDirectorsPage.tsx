...
import { Quote, GraduationCap, Award, Star, Users, Play, Volume2, VolumeX } from "lucide-react";
...
export default function OurDirectorsPage() {
  ...
  const [principalMuted, setPrincipalMuted] = useState(true);

  const togglePrincipalSound = () => {
    const v = principalVideoRef.current;
    if (!v) return;
    v.muted = !v.muted;
    setPrincipalMuted(v.muted);
  };
        ...
        <div className="container mx-auto px-4 md:px-6 max-w-4xl">
          ...
          <motion.div {...fadeUp(0)} className="text-center mb-10">
            <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-widest mb-4"
              ...
              <Play size={11} style={{ fill:"#059669" }} />
          ...
          <motion.div {...fadeUp(0.15)}
            ...
            <div className="h-1.5" style={{ background:"linear-gradient(90deg,#2563EB,#10B981,#8B5CF6)" }}/>
            ...
            <div className="p-4 md:p-6">
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  <p className="text-foreground font-semibold text-sm leading-none">Mr. Atul Sharma</p>
                  <p className="text-muted-foreground text-xs mt-0.5">{isHindi ? "प्रिंसिपल, द माइलस्टोन Sr. Sec. School" : "Principal, The Milestone Sr. Sec. School"}</p>
                </div>
              ...
              <div className="relative w-full rounded-2xl overflow-hidden"
                style={{ background:"#000", aspectRatio:"16/9", boxShadow:"0 4px 20px rgba(0,0,0,0.15)" }}>
                <video
              ...
              <div className="mt-4 flex flex-wrap items-center justify-between gap-3">
                ...
                <div className="flex items-center gap-3">
                  <button
                    ...
                    style={{ background: principalMuted ? "rgba(0,0,0,0.06)" : "rgba(16,185,129,0.10)", border: principalMuted ? "1px solid rgba(0,0,0,0.12)" : "1px solid rgba(16,185,129,0.30)", color: principalMuted ? "#6b7280" : "#059669" }}>
                    {principalMuted
                      ? <><VolumeX size={13}/> {isHindi ? "आवाज़ बंद" : "Sound Off"}</>
                      : <><Volume2 size={13}/> {isHindi ? "आवाज़ चालू" : "Sound On"}</>}
                  </button>
                  <div className="flex items-center gap-2">
                    <GraduationCap size={13} className="text-emerald-600"/>
                    <span className="text-muted-foreground text-xs">{isHindi ? "M.A., B.Ed. | 20+ वर्ष" : "M.A., B.Ed. | 20+ Years"}</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            {boardMembers.map(({ name, role, note, accent }, i) => (
              <motion.div key={name} {...fadeUp(i * 0.1)}
                ...
                <div className="h-1.5" style={{ background:`linear-gradient(90deg,${accent},${accent}66)` }}/>
